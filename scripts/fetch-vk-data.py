...
def crawl_graph(start_node_id, depth, process_data_and_get_ids_fn):
    global queue, should_continue_execution, execution_stopped, TIMER_SEC

    start_time = time.time()

    if len(queue) == 0:
        queue = [(start_node_id, 0)]
    while len(queue) > 0 and should_continue_execution:
        node_id, cur_depth = queue[0]
        if cur_depth >= depth:
            return
        print 'started processing of', node_id, 'on depth', cur_depth
        queue = queue[1:]
        new_node_ids = process_data_and_get_ids_fn(node_id)
        print 'new ids count:', len(new_node_ids), 'total ids count:', len(processed_friend_ids)
...
def wait_for_user_input():
    global should_continue_execution, execution_stopped
    while not execution_stopped:
...
def stop_execution():
    ...
    while not execution_stopped:
        pass
    print 'persisting execution state...'
    persist_execution_state()
    print 'done'

parser = argparse.ArgumentParser('Fetching university data from vk')
parser.add_argument('--continue', help='continue execution using previously persisted state')
...
should_load_execution_state = False
if args['continue'] is not None:
    should_load_execution_state = True
...
t = threading.Thread(target=wait_for_user_input)
t.setDaemon(True)
t.start()
